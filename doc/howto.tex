...
Il y a 2 façons de changer le .bin :
    ...
    \subsubsection{Façon 2 :}
        Avec une version du driver <= 0.5, vous pouvez effectuer ceci :\\
...
\end{verbatim}
        ...
        Pour les autres, voir eciconf.sh (ou eciconftxt.sh).\\
        Les serveurs DNS sont stockés dans le fichier providers.db (version driver CVS ou >= 0.6).
...
\end{verbatim}
    ...
    Cas 2 : vous avez un probl\`eme d'authentification (PPP ne le dit pas toujours explicitement)
        ...
        Corrigez vos scripts :\\
            ...
            et\\
            \textit{/etc/ppp/chap-secrets} (Wanadoo et Club Internet "Full is beautiful") ou \textit{/etc/ppp/pap-secrets} (Club Internet except\'e "Full is beautiful")\par
