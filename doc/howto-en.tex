...
\author{Bertrand Rougier, Thierry De Baere, Florent Manens, Benoît Papillault, FlashCode\\
...
\end{verbatim}
    ...
    There are two ways to remove that module :

    \textbf{1)} With eciconf.sh : unplug your modem  then in the eciconf.sh interface
    click on "Remove dabusb module".

    \textbf{2)} Type the following commands replacing VERSION by your Linux kernel version:
...
There are two ways to change your .bin :
    ...
    \subsubsection{1 :}
        ...
        it is possible to change it in eciconf.sh
