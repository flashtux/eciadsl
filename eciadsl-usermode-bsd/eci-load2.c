...
void read_endpoint(pusb_device_t dev, int epnum, int option_verbose)
{
	...
	ep = pusb_endpoint_open(dev, epnum);
	if (ep == NULL)
	{
	...
	for (i = 0; /* i < 100 */; i++)
	{
		...
		*/
		...
		ret = pusb_endpoint_int_read(ep, lbuf, sizeof(lbuf));
		
		if (ret < 0)
		{
            perror("error reading interrupts\n");
			break;
		}
			  ...
			  int option_verbose, uint option_timeout)
{
	...
	if (pusb_device_set_interface(dev, 0, 4) < 0)
	{
...
/*
	{
		...
		ep_data = pusb_endpoint_open(dev, 0x2);
		if (ep_data == NULL)
		{
