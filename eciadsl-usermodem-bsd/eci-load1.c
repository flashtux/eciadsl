			  ...
			  int option_verbose)
{
	...
	if (pusb_device_set_configuration(dev, 1) < 0)
	{
		printf("can't set configuration 1\n");
		pusb_device_close(dev);
		fclose(fp);
		return(0);
	}

	if (pusb_device_set_interface(dev, 0, 1) < 0)
	{
...
*/
    ...
    printf("using VID1=0x%04x PID1=0x%04x VID2=0x%04x PID2=0x%04x\n",
           vid1,pid1,vid2,pid2);
